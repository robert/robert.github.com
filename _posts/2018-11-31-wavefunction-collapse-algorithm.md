...
published: false
---
The Wavefunction Collapse Algorithm teaches your computer how to riff. It takes in an archetypical input, and produces procedurally-generated outputs that look like it.

<img src="/images/wfc-examples.png" />
*([Source][unity-wfc])*

It is most commonly used to create images, but is also perfectly capable of building [towns][unity-wfc], [skateparks][skater], and [terrible poetry][poetry].
...
This symmetry-handling makes for better output images, but more complex code. To keep things simple whilst we're still learning, let's move on from the Simple Tiled Model to consider an even simpler form of Wavefunction Collapse, which I'll call an *Even Simpler Tiled Model*.
...
When analyzing the example iamge, we also need to record the frequency at which the different tiles appear. We will later use these numbers as weights when selecting which square's wavefunction to collapse, and when choosing which tile to assign to a square when it is being collapsed.
