...
The rest of this post shows how you probe your own black-box internet of things (IOT) devices by intercepting their network traffic. These steps should work for almost any smart, internet-enabled device, be it a crib, a fridge, a bathroom, a lightbulb, or a sandwich.
...
I've described DNS-spoofing is much more detail [in parts 1 and 2 of my series on building your own TCP proxy](/2018/08/31/how-to-build-a-tcp-proxy-1/). [Part 2 of that series](/2018/08/31/how-to-build-a-tcp-proxy-2/) has an example `scapy` script that performs the above spoofing logic. For `INSERT_YOUR_LAPTOPS_IP_ADDRESS_HERE`, you will need to use your laptop's IP address on the *interface* corresponding to the network that it is the router for. For me this interface was called `bridge100`. You may need to do some investigating to figure out your equivalent.
