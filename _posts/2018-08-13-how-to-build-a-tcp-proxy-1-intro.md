---
title: "How to build a TCP proxy #0: Intro"
layout: post
---
It was a weird and smoky afternoon in San Francisco. My downstairs neighbors had apparently never heard of vaporizers, and the Sierra Nevada was on fire. I had planned to spend the rest of the day attacking the user-location features of popular dating apps. I wanted to see whether any of them were vulnerable to attacks that could reverse engineer the position of a victim user. My plan was to spoof thousands of requests to each target app, pretend that each request was sent from a different location, and each time ask the app how far away my target was from my current, faked position. I would combine the results using maths and ingenuity to try and shake out an exact location.
...
We are going to build a particular type of proxy known as a "man-in-the-middle" (MITM). MITM proxies are commonly used by penetration testers in order to read, log, and modify the data that a client (like a smartphone) exchanges with a remote server (like a dating app).
...
This is more than just an oddity. We can program our fake DNS server to send back any DNS response we like. We can even make it send back total lies. Suppose that our DNS server receives a DNS request for a hostname whose requests we want to route through our proxy (say, `targetapp.com`). We will program our server to respond, not with `targetapp.com`'s real IP address, but with the local IP address of your laptop.

<img src="/images/tcp-1-full-layout.png" />

Your phone won't know that we are lying, and won't see anything wrong with our response. It will accept that `targetapp.com` resolves to your laptop's local IP address, and dispatch any data that it wants to send to `targetapp.com` to your laptop.
...
HTTP/S proxies like Burp don't need any DNS jiggery-pokery in order to get your smartphone to send data to them. Why does our TCP proxy?
...
All sensible smartphones take advantage of these features, and work well with HTTP/S proxies. Your smartphone has a system setting that allows you to specify a proxy that it should use for all of its HTTP requests. In order to use an HTTP MITM proxy like Burp, you first connect your laptop and smartphone to the same network, and then tell your smartphone to use your laptop's local IP address as an HTTP proxy. You open your Burp proxy on your laptop, and your smartphone obediently forwards all of its HTTP/S traffic to your laptop. Burp is instantly up and running.
...
The second piece of our system it the proxy server itself. This will be a server that runs on your laptop. It will accept traffic from your smartphone (with a little help from our fake DNS server), forward it to the remote server, and send any response that it gets back to your smartphone.
...
## 2.3 TLS and a Fake Certificate Authority
...
Since we don't have any real, CA-signed TLS certificates, we're going to make our own, fake CA, called "Robert's Reliable Certificate Authority". We will generate a signing certificate for our CA, and install this certificate on your smartphone as a *trusted CA*. This will cause your smartphone to trust both our CA, and, more importantly, all other certificates that our CA signs. Whenever your smartphone asks our proxy to perform a TLS handshake, we'll check the hostname that your smartphone believes it is talking to. We'll quickly generate a certificate for that hostname (say, `api.targapp.com`), sign it with our fake CA, and present it to your smartphone. Your smartphone will see that this certificate for `api.targapp.com` is signed by "Robert's Reliable Certificate Authority", see that this reputable organization is in its list of trusted CAs, and happily complete a TLS handshake with our proxy.

With a TLS connection between your smartphone and our proxy successfully established, the rest of our system will work exactly as planned. We'll be done.

## 3. The Project

This project is divided into 3 sections, each of which describes how to build one of 3 components:
