...
*Off-The-Record Messaging* (OTR) is a cryptographic protocol that aims to replicate the privacy properties of casual, in-person conversation. The goal of the OTR protocol is that no one can overhear an OTR exchange, and if someone somehow does, they can't prove the legitimacy of anything that they heard, and they certainly can't decrypt and read reams of historical messages. Any fool with decades of experience in high-grade cryptography can design a communication protocol that is secure when things go right. OTR focuses on what happens when things go wrong.
...
Cryptography is a very precise field so I'll start by quoting a technical definition of forward secrecy to show that I'm also a very precise person. We don't need to know the mathematics of forward secrecy, but a quick synopsis will be important to understanding OTR:
...
Finally, the sender and recipient must:

* Forget the encryoption session key in order to preserve forward secrecy
...
A secret key agreed using Diffie-Hellman can be used with any symmetric cipher. In OTR, Alice and Bob use a *stream cipher with AES in counter mode*. Once again, the details of what this means and how it works are not important to us. We can treat the cipher as a black box that Alice and Bob can combine with their symmetric key to encrypt or decrypt a message. What is important to us is that OTR uses a stream cipher because it is *malleable*, meaning that it is particularly *easy* for an attacker to tamper with. We'll see why this counter-intuitive property is desirable shortly.
...
That's how you send an OTR message, but some of these steps raise more questions than they answer. Why exactly does OTR use HMAC signatures? Why does it use a malleable cipher? And why do participants publish their private signing keys?

Let's find out.

## Key insights of OTR

### Why does OTR use HMAC signatures instead of an asymmetric algorithm?
...
OTR uses HMAC signatures. Unlike PGP signatures, HMAC signatures are both created and verified using a single shared secret key, known to both the signer and the verifier. The signer creates the signature by passing their message and the shared secret key into the HMAC algorithm. The verifier verifies the signature by performing the same operation and comparing their result with the given signature.
...
To see why this is oddly desirable, remember that the worst-case situation that OTR tries to prevent at all costs is the one in which an attacker:
